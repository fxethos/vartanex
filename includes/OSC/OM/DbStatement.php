...
// use the optional argument
    ...
    public function getPageSetLinks($parameters = null)
    {
		...
		//print_r($parameters);
		$pagerange = MAX_DISPLAY_PAGE_LINKS;
	...
	if (($this->page_set > 1) && ($number_of_pages > 1)){
		$output .= '<li><a href="' . OSCOM::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . (1), $request_type) . '" title=" ' . PREVNEXT_TITLE_FIRST_PAGE . ' ">&laquo;</a></li>';
	}
	// previous button - not displayed on first page
        if ($this->page_set > 1) {			
            $output .= '<li><a href="' . OSCOM::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . ($this->page_set - 1), $request_type) . '" title=" ' . PREVNEXT_TITLE_PREVIOUS_PAGE . ' ">&lt;</a></li>';
        } else {
            $output .= '<li class="disabled"><span>&laquo;</span></li>';
        }
	//echo $this->page_set;
	// check if number_of_pages > $max_page_links
	//echo $this->page_set / $this->page_set_total_rows;
	$cur_window_num = (int)($this->page_set / $pagerange);
	if ($this->page_set % $pagerange) {
		$cur_window_num++;
	}

	$max_window_num = (int)($number_of_pages / $pagerange);
	if ($number_of_pages % $pagerange) {
		$max_window_num++;
	}
...
// previous window of pages
        if ($cur_window_num > 1) {
            $output .= '<li><a href="' . OSCOM::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . (($cur_window_num - 1) * $pagerange), $request_type) . '" title=" ' . sprintf(PREVNEXT_TITLE_PREV_SET_OF_NO_PAGE, $pagerange) . ' ">...</a></li>';
        }

// page nn button
        for ($jump_to_page = 1 + (($cur_window_num - 1) * $pagerange); ($jump_to_page <= ($cur_window_num * $pagerange)) && ($jump_to_page <= $number_of_pages); $jump_to_page++) {
...
// next window of pages
        if ($cur_window_num < $max_window_num) {
            $output .= '<li><a href="' . OSCOM::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . (($cur_window_num) * $pagerange + 1), $request_type) . '" class="pageResults" title=" ' . sprintf(PREVNEXT_TITLE_NEXT_SET_OF_NO_PAGE, $pagerange) . ' ">...</a></li>&nbsp;';
        }

// next button

        if (($this->page_set < $number_of_pages) && ($number_of_pages != 1)) {
            $output .= '<li><a href="' . OSCOM::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . ($this->page_set + 1), $request_type) . '" title=" ' . PREVNEXT_TITLE_NEXT_PAGE . ' ">&gt;</a></li>';			
        } else {
            $output .= '<li class="disabled"><span>&raquo;</span></li>';
        }
		
		if (($this->page_set < $number_of_pages) && ($number_of_pages != 1)){
			$output .= '<li><a href="' . OSCOM::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . ($number_of_pages), $request_type) . '" title=" ' . PREVNEXT_TITLE_LAST_PAGE . ' ">&raquo;</a></li>';			
		}
